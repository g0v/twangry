var request = require('request');
var cheerio = require('cheerio');
var fs = require('fs');
var url = require('url');
var timeline = require('./timeline');
var dateparser = require('./zhdateparser');

function wiki() {
    this._scheme = 'http://';
    this._domain = 'zh.wikipedia.org';
    this.base_url = this._scheme + this._domain +'/';
    this._api = 'w/api.php?action={action}&prop={prop}&format=json&titles=';
    this._render = 'w/index.php?&action=render&variant=zh-tw&title=';
    this._minLength = 25;
    this.timeline = new timeline;
    this.timeline.init();
}

wiki.prototype.request = function(key, callback){
  var uri = this.base_url+this._render+key;
  var thiswiki = this;
  request(uri, function (error, response, data) {
    if(error) { 
      callback(true);
    }
    else{
      var dom =  cheerio.load(data);
      thiswiki.parse(dom, key);
      callback(null);
    }
  });
}

wiki.prototype.requestCoord = function(key, callback){
  var api = this._api.replace('{action}', 'query').replace('{prop}', 'coordinates');
  var thiswiki = this;
  request(this.base_url + api+key, function(e, r, d){
    if(!e && d){
      var regex = /"lat":([^,]+),"lon":([^,]+)/;
      if(regex.test(d)){
        var coord = d.match(regex);
        coord.shift();
        var src = 'https://maps.google.com.tw/maps?t=h&z=8&iwloc=A&q='+coord.join(',');
        var asset = thiswiki.timeline.asset(src, '', key);
        thiswiki.timeline.set('asset', asset);
      }
      callback(null);
    }
    else{
      callback(e);
    }
  });
}

wiki.prototype.parse = function($, key){
  // prepare clean body
  $('#spoiler').remove();
  $('.mw-editsection').remove();
  $('.ambox').remove();
  $('.navbox').remove();
  var img = $('a.image').find('img').eq(0);
  $('.infobox').remove();
  $('.rellink').remove();
  
  // prepare headline
  var summary, src, desc;
  summary = $('p').eq(0).text().replace(/\[[0-9]+\]/g, '');
  if($('#coordinates').length){
    $('#coordinates').remove();
    summary = $('p').eq(0).text().replace(/\[[0-9]+\]/g, '');
  }
  else{
  /*
    $('a.external').each(function(){
      // trying to find video 
      var href = $(this).attr('href');
      if(href.match('youtube.com')){
        src = href;
        desc = $(this).text();
        return;
      }
    });
  */
    if(!src){
      var desc = $('.thumbinner').eq(0).text().replace(/(\r\n|\n|\r)/gm, '');
      src = img.attr('src');
      if(src){
        src = this.imageOrigin(src);
      }
    }
    if(src){
      var asset = this.timeline.asset(src, '', desc);
      this.timeline.set('asset', asset);
    }
  }

  $('p').eq(0).remove();
  this.timeline.set('headline', key);
  this.timeline.set('text', summary);

  // parse date in the body
  // $('.references').remove(); // berfore parse, remove reference first.
  if(key.match(/[1-2][0-9]{3}年[0-9]{1,2}月/)){
    this.parseDatePage($, key); 
  }
  else{
    this.parseThumb($, key);
    this.parseChineseDate($, key);
  }
}

wiki.prototype.parseThumb = function($, key){
  var thiswiki = this;
  $('.thumbinner').each(function(){
    // clear
    $(this).find('.magnify').remove();

    // image
    var src = $(this).find('img').attr('src');
    src = thiswiki.imageOrigin(src);
    src = 'http:'+src;
    var asset = thiswiki.timeline.asset(src, '', '');

    // text
    var caption = $(this).children('.thumbcaption');
    thiswiki.parseChineseDate(caption, key, caption.text(), asset);
    $(this).remove();
  });
}

wiki.prototype.parseChineseDate = function($, key, text, asset){
  var html = text ? '<div>'+text+'</div>' : $.html();

  dateparser.parse(html);
  var matches = dateparser.dump();
  var thiswiki = this;
  if(matches){
    matches.forEach(function(date) {
      var tag;
      if(date.summary.match(/<sup>.+(出版|參考文獻|參考資料|參見|外部連結|內文註釋|資料來源|註釋|相關參見).+<\/sup>/)){
        return false;
      }
      date.summary = date.summary.replace(/<a[^>]*>((?:\d+年)?(?:\d+月)?(?:\d+日)?)<\/a>/g, '$1');
      date.summary = date.summary.replace(/rel="([^"]+)"/g, 'href="'+this.base_url+'wiki/'+key+'#$1"');

      var d = cheerio.load(date.summary);
      tag = d('sup > a.sup').eq(0).text().replace(/\[|\]/g,'');
      // section link
      // get asset
      if(!asset){
        if(d('.reference')){
          var ref = d('.reference a').attr('href');
          if(typeof(ref) === 'string' && ref.match(/#cite_note-\d+/)){
            var ahref = $(ref).find('a.external');
            if(ahref){
              asset = thiswiki.timeline.asset(ahref.attr('href'), '', ahref.text());
            }
          }
          d('.reference').remove();
        }

        if(!asset){
          d('a.new').each(function(){
            d(this).after('<span>'+d(this).text()+'</span>');
            d(this).remove();
          });
          d('a').each(function(){
            if(!asset && !d(this).hasClass('sup')){
              var wikihref = d(this).attr('href') || '';
              var wikitext = d(this).text();
              if(wikihref.match(this._domain+'/wiki/')){
                asset = thiswiki.timeline.asset(wikihref, '', d(this).attr('title'));
                return;
              }
            }
          });
        }
      }

      // get text
      if(!text){
        var text = d.html();
        text = text.replace(/\[[0-9]+\]/g, '');
        text = text.replace(/\[來源請求\]/g, '');
      }

      var over= text.replace(/(<[a-z]+\s[^>]+>|<\/[a-z]+>)/gi, '');
      if(over.length > thiswiki._minLength && !over.match('ISBN')){
        thiswiki.timeline.setDate(date.startDate, date.endDate, date.title, text, asset, tag);
      }

      // clear for next loop
      asset = text = null;
    });
  }
}

wiki.prototype.parseDatePage = function($, key){
  var year = key.match(/[1-2][0-9]{3}年/);
  var thiswiki = this;
  $('h2').each(function(){
    var date = year + $(this).text();
    startDate = date.replace(/(月)|(日)|(年)/g, ',').replace(/,$/, '');
    var content = $.html($(this).next());
    thiswiki.timeline.setDate(startDate, date, content);
  });
}

wiki.prototype.imageOrigin = function(src){
  var l = src.lastIndexOf('/');
  return src.substr(0, l).replace('/thumb', '');
}

wiki.prototype.updateArticle = function(){
  var limit = 1;
  var base_url = 'http://fact.g0v.tw/';
  var dir = 'public/cache/wiki/'; // your directory
  var files = fs.readdirSync(dir)
    .map(function(v) { 
      return {
        name:v,
        time:fs.statSync(dir + v).mtime.getTime()
      }; 
    })
    .sort(function(a, b) { return a.time - b.time; })
    .map(function(v) { return v.name; });

  for(i=0; i<limit; i++){
    var url = base_url+'wiki/'+files[i]+'?nocache=1';
    request(url, function (error, response, body) {
      console.log(url);
      // empty
    });
  }
}

module.exports = wiki;
