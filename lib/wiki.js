  ...
  request(wiki.base_url + api+key, function(e, r, d){
    if(!e && d){
      ...
      if(regex.test(d)){
        ...
        coord.shift();
        var src = 'https://maps.google.com.tw/maps?t=h&z=9&iwloc=A&q='+coord.join(',');
...
wiki.parse = function($, key){
  ...
  if($('#coordinates')){
    ...
    summary = $('p').eq(0).text().replace(/\[[0-9]+\]/g, '');
    /*
    wiki.requestCoord(key, function(coord){
      if(coord){
        src = 'https://maps.google.com.tw/maps?t=h&z=13&iwloc=A&q='+coord;
        console.log(src);
        var asset = timeline.asset(src, '', key);
        timeline.set('asset', asset);
      }
