...
wiki.parseThumb = function(dom){
  dom('.thumbinner').each(function(){
    ...
    wiki.parseChineseDate(caption, asset);
  });
}
wiki.parseChineseDate = function(dom, asset){
  var content = dom.html();
  var matches = content.match(/(<a[^>]+>|)[1-2][0-9]{3}年(<\/a>|代|底|)([0-9]{1,2}月|)([0-9]{1,2}日|).+(<\/(div|p|li|ul)>|<br\s?\/>|。|\n)/g);
  if(matches){
