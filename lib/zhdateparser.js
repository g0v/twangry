...
zhdateparser.parse = function(html){
  ...
  while(m = pattern.exec(html) ){
    ...
    var paragraph = m[0].replace(/((?:。|；)(?:<sup.*?<\/sup>)*)/g, "$1@@@"); // add placeholder for sentence
...
zhdateparser.extractDate = function(m, paragraph, html, mindex){
  ...
  if(datestr){
    ...
    var str = paragraph.substr(0, m.index);
    var s = 0;
    ...
    */
    var pre = post = '';
    summary = '<span title="'+pre.replace(/<[^>]+>/g, '')+'">...</span>' + summary + text + '<span title="'+post.replace(/<[^>]+>/g, '')+'">...</span>';
    ...
    summary = '<sup>'+sup.reverse().join('&raquo;')+'</sup><br />' + summary;
