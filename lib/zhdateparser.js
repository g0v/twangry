...
zhdateparser.parse = function(html){
  ...
  if(refTime){
    var year = refTime.match(/[1-9][0-9][0-9][0-9]/g);
    if(year){
      lyear=year[0]+'年';
    }
  }
  ...
  pattern = /(<a[^>]+>)?(西元)?([1-9][0-9]{0,3}年?)?(－|-|~|～)?([1-9]?[0-9]{0,3}(<\/[^>]+>)?(年代|年|底)(<\/[^>]+>)?([0-9]{1,2}月(<\/[^>]+>)?|)([0-9]{1,2}日|)).+(<\/(li|p|div|ul)>)/g;
...
zhdateparser.extractDate = function(m, paragraph, html, mindex, refhtml){
  ...
  if(datestr){
    ...
    if(datestr.match(/(西元)?(.{0,1}[1-9][0-9]{0,3}.{0,1})(－|-|~|～|\/|年)(.{0,1}[0-9]{1,2}.{0,1})(－|-|~|～|\/|月)(.{0,1}[0-9]{1,2}.{0,1})(日)/)){ // 1999年9月9日
