 ...
 * @constructor
 */
function Zhdateparser() {
  this.option = {lookback:180,delimeter:','};
  this.dates = [];
}
 ...
 */
Zhdateparser.prototype.set = function(date){
 ...
 */
Zhdateparser.prototype.dump = function(){
  var r = this.dates;
  this.dates = [];
  return r;
};
 ...
 */
Zhdateparser.prototype.parse = function(html){
  // purge unecessery html
  html = html.replace(/(\n|\r|\r\n)+(<\/(?:div|p|ul|li)>)/g, '$2');
  html = html.replace(/(<(?:div|p|ul|li|br)[^>]*>)(\n|\r|\r\n)+/g, '$1');
  html = html.replace(/<td.*>\d+<\/td>/g, '');
  ...
  while(m = pattern.exec(mainhtml)) {
    ...
    var range = mainhtml.substr(dateidx, 1000);
    range = range.replace(/(\r\n|\n|\r)/gm," ");
    range = range.replace(/<\/(p|div|li|ul)>.*$/g, "");
  ...
  html = mainhtml.replace(/((西元)?([1-2]\d{3}年?(－|-|~|～)[1-2]\d{3}年)|\^[1-2]\d{3}年|[1-2]\d{3}年)/g, "@@@$1"); // add placeholder for year
  // note that we need to remove two close date in string
...
Zhdateparser.prototype.extractYearFromReference = function (paragraph, refhtml){
  var refResult=[];
  var refTime='';
 ...
 */
Zhdateparser.prototype.extractDate = function(m, paragraph, html, mindex, refhtml){
  var text = m[0];
  ...
  if(datestr){
    ...
    // add auto year mark
    paragraph = paragraph.replace(/\^(\d+年)/g, '<abbr class="autoyear" title="由前後文自動判斷的年份">$1</abbr>');
    var psplit = paragraph.replace(/<\/?(p|div)[^>]*>\n?/g, '').replace(/@@@$/, '').split('@@@');
    if(psplit){
      ...
      if(start > 0) {
        if(psplit[start-1].length < 12){
          psplit[start] = psplit[start-1] + psplit[start];
          psplit[start-1] = '';
        }
      }
      if(end < psplit.length-1) {
        if(psplit[end+1].length < 12 || psplit[end].length < 12 ){
          psplit[end] = psplit[end] + psplit[end+1];
          psplit[end+1] = '';
        }
      }
      for(var i=0; i<psplit.length; i++){
        if(i < start){
          summary.before += psplit[i];
        }
        else
        if(i <= end){
          summary.main += psplit[i];
        }
        else{
          summary.after += psplit[i];
          // stop on first paragraph prevent too long
          if(summary.after.match(/。$/)){
            break;
          }
        }
      }
    ...
    summary.breadcrumb = sup.reverse();

    // prepare and store date object
    date.startDate = startDate;
    date.endDate = endDate ? endDate : '';
    date.summary = summary;  // an array of content
    date.title = title;
 ...
 */
Zhdateparser.prototype.insertYear = function(html, index, string){
  if (index > 0){
