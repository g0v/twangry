...
Wiki.prototype.appendTimeline = function (src) {
  var w = this;
  if (fs.existsSync('public/cache/wiki/' + this.key + '.json')) {
...
Wiki.prototype.parseChineseDate = function($, text, asset){
  ...
  if(matches){
    matches.forEach(function(date) {
      ...
      date.summary = date.summary.replace(/rel="([^"]+)"/g, 'href="'+this.base_url+'wiki/'+this.key+'#$1"');
