...
function render_event_page(key) {
  var articles = get_articles(key);

  var jsonpath = '/wiki/'+articles.join('+')+'.json';
...
function render_event_json(key) {
  ...
  var res = this.res;
  var wikis = [];
  get_articles(key).forEach(function (k) {
    if (fs.existsSync('public/cache/wiki/' + k + '.json')) {
      wikis.push(function (callback) {
        var tl = timeline();
        tl.json = JSON.parse(fs.readFileSync('public/cache/wiki/' + k + '.json'));
        callback(null, tl);
      });
    }
    else {
      wikis.push(function (callback) {
        wiki(k).request(callback);
      });
      wikis.push(function (callback) {
        var wp = wiki(k);
        setTimeout(function () {
