...
function render_event_json(key) {
  ...
  key.split(/\+/).forEach(function (k) {
    ...
    if (s.length == 1) {
      ...
      sources['wikipedia'].get(k).appendTimeline(timelines, req);
    } else {
      sources[s[0]].get(s[1]).appendTimeline(timelines, req);
    }
  });
...
route.watch = function(req){
  for (var src in sources) {
    if (sources[src].get().updateCache) {
      setInterval(sources[src].get().updateCache, 900*1000);
    }
  }
  var p = url.parse(decodeURIComponent(req.url));
