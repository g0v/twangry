...
function index() {
  ...
  this.spreadsheet_uri = 'https://spreadsheets.google.com/feeds/list/0AuwTztKH2tKidGZ2cEdVY19PZEpzRWVJWWZOeUI1Y0E/od6/public/values?alt=json';
}
...
index.prototype.request_json = function (route, res) {
  ...
  request(this.spreadsheet_uri, function (error, response, data) {
    if(!error){
      idx.parseFront(data);
      var events = idx.timeline.json.timeline;
...
index.prototype.parseFront = function(data){
  ...
  json.forEach(function(d){
    ...
    thisindex.timeline.setDate(startDate, endDate, d['gsx$headline'].$t, d['gsx$text'].$t, asset, t, timestamp);
  });
  var sorted_years = [];
  var k;
  for(k in years){
